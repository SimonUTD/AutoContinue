...
- **错误检测**: 基于 Detector 适配器做结构化状态/错误检测
...
https://raw.githubusercontent.com/SimonUTD/AutoContinue/main/install-by-agent.md
```
...
从 [GitHub Releases](https://github.com/SimonUTD/AutoContinue/releases/latest) 下载对应平台的预编译版本：
...
```bash
# 克隆仓库
git clone https://github.com/SimonUTD/AutoContinue.git
cd AutoContinue
...
4. **状态检测**: 通过 CLI 对应的 Detector 判定 Busy / Idle / Error / Unknown
5. **自动响应**:
   - 检测到错误 → 发送重试提示词
   - 正常静默 → 发送继续提示词
...
| 其他工具 | `GenericDetector` | 输出文本模式匹配 + 静默超时 fallback |
